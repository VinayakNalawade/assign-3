...
const giveInitialState = () => {
  const selected = tabsList[0]
  const score = 0
  const time = 60
  const matchlist = [...imagesList]
  const matchimg = matchlist.shift()
  const isGameOn = true

  return {selected, score, time, matchlist, matchimg, isGameOn}
}

const initialState = giveInitialState()
...
class MatchGame extends Component {
  ...
  onClickImage = imgItem => {
    const {score, isGameOn, matchlist, matchimg} = this.state

    if (matchimg.id === imgItem.id) {
      const updatedscore = score + 1

      if (updatedscore === imagesList.length) {
        clearInterval(this.timerId)

        this.setState({isGameOn: !isGameOn, score: updatedscore})
      } else {
        const newmatchlist = [...matchlist].sort(() => Math.random() - 0.5)
        const newmatchimg = newmatchlist.pop()
        console.log(newmatchlist)
        this.setState({
          score: updatedscore,
          matchimg: newmatchimg,
  ...
  render() {
    const {selected, score, time, matchimg, isGameOn} = this.state

    const displayList = imagesList.filter(
      each => each.category === selected.tabId,
    )
    ...
    return (
      <div className="mainContainer">
        <nav className="navbar">
          ...
          <ul className="scoretimeContainer">
            ...
            <li className="timeContainer">
              ...
              />
              <p className="timerCount">{time} sec</p>
            </li>
          </ul>
        </nav>
        <div className="pageContainer">
          {isGameOn ? (
            <>
              <img className="matchimg" alt="match" src={matchimg.imageUrl} />
            ...
            <div className="resultContainer">
              ...
              />
              <p className="resultheading">YOUR SCORE</p>
